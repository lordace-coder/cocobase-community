...
def check_and_increment_api_usage(
    ...
    try:
        ...
        pipe.get(redis_key)
        pipe.incr(redis_key)
        pipe.get(redis_sync_key)
        results = pipe.execute()
        
        current_usage_str = results[0]
        # Handle bytes from Redis (redis-py returns bytes by default)
        if isinstance(results[1], bytes):
            new_usage = int(results[1].decode('utf-8'))
        elif results[1] is not None:
            new_usage = int(results[1])
        else:
            new_usage = 1
        ...
        if current_usage_str is None:
            ...
            current_usage = _load_usage_from_db(project.id, current_month, db, redis_client, redis_key)
            new_usage = current_usage + 1
            redis_client.set(redis_key, new_usage)
        ...
        if new_usage - last_sync_count >= SYNC_THRESHOLD:
            ...
            # Update last sync marker atomically
            pipe = redis_client.pipeline()
            pipe.set(redis_sync_key, new_usage)
            pipe.expire(redis_sync_key, 60 * 60 * 24 * 60)
            pipe.execute()
...
def _load_usage_from_db(
    ...
    """
    usage_record = db.query(ApiUsageCounter).filter(
        ApiUsageCounter.project_id == project_id,
        ApiUsageCounter.month == month
    ).first()
    
    if usage_record:
        # Found in database - restore to Redis using pipeline
        current_usage = usage_record.request_count
        
        pipe = redis_client.pipeline()
        pipe.set(redis_key, current_usage)
        pipe.set(f"{redis_key}{REDIS_SYNC_KEY_SUFFIX}", current_usage)
        pipe.expire(redis_key, 60 * 60 * 24 * 60)
        pipe.expire(f"{redis_key}{REDIS_SYNC_KEY_SUFFIX}", 60 * 60 * 24 * 60)
        pipe.execute()
        ...
        pipe.set(f"{redis_key}{REDIS_SYNC_KEY_SUFFIX}", 0)
        pipe.expire(redis_key, 60 * 60 * 24 * 60)
        pipe.expire(f"{redis_key}{REDIS_SYNC_KEY_SUFFIX}", 60 * 60 * 24 * 60)
        pipe.execute()
...
def notify_limit_warning(user: User, project, resource_type: str, current_usage: int, limit: int, severity: str):
    """Send warning notification."""
    percentage = (current_usage / limit) * 100
    emoji = {"info": "ℹ️", "warning": "⚠️", "critical": "🚨"}[severity]
    print(f"{emoji} {severity.upper()}: {project.name} - {resource_type}: {current_usage}/{limit} ({percentage:.0f}%)")
    pass